...
def set_data_structure(data):
    ...
    for row in data:
        ...
        for column_index in range(1,len(row)):
            vertice_distance_dict[(row[0][0], row[column_index][0])] = row[column_index][1]
...
def compute_shortest_path(source_vertex,path_dict,vertice_list,vertice_distance_dict):
    ...
    # while not X = V:
    while not sorted(vertices_processed) == sorted(vertice_list):           # change : len(X) = len(V)
        ...
        for vertice_pair,distance in vertice_distance_dict.items():
            print('--------------Before :--------------')
            print("vertice_pair :",vertice_pair,"distance :",distance)
            l_v_w = 0
            # print("tail :",vertice_pair[0],"head :",vertice_pair[1])
            print("vertex_short_dist[vertice_pair[0]] :",vertex_short_dist[vertice_pair[0]],"path_dict[vertice_pair[0]][vertice_pair[1]] :",path_dict[vertice_pair[0]][vertice_pair[1]])
            l_v_w = vertex_short_dist[vertice_pair[0]] + path_dict[vertice_pair[0]][vertice_pair[1]]
            print("l_v_w :", l_v_w, " vertex_short_dist[head_vertex] :", vertex_short_dist[vertice_pair[1]])
            if vertex_short_dist[vertice_pair[1]] >l_v_w:
                if ((vertice_pair[0] in vertices_processed) and (not vertice_pair[1] in vertices_processed)):
                    # #     - add w* to X
                    #         vertices_processed.append(w*)
                    vertices_processed.append(vertice_pair[1])
                    # #     - set A[w*] = A[v*]+ l[v*_w*]
                    #         A[w*] = A[v*]+ l[v*_w*]
                    print("Before assiging short distance",vertex_short_dist[vertice_pair[1]])
                    vertex_short_dist[vertice_pair[1]] = l_v_w
                    print("After assiging short distance", vertex_short_dist[vertice_pair[1]])
                    # #     - set B[w*] = B[v*]u(v*_w*)
                    #         B[w*] = B[v*]+v*_w*
                    print("Before assiging short path", vertex_path_dict[vertice_pair[1]])
                    vertex_path_dict[vertice_pair[1]] = vertex_path_dict[vertice_pair[0]] + [vertice_pair[1]]
                    print("After assiging short path", vertex_path_dict[vertice_pair[1]])
                elif vertice_pair[1] in vertices_processed:
                    vertex_short_dist[vertice_pair[1]] = l_v_w
                    vertex_path_dict[vertice_pair[1]] = vertex_path_dict[vertice_pair[0]] + [vertice_pair[1]]
                # print("vertices_processed :", vertices_processed)
                # print("vertex_short_dist :", vertex_short_dist)
                print("After  l_v_w :", l_v_w, " vertex_short_dist[head_vertex] :", vertex_short_dist[vertice_pair[1]])
            # print("vertex_path_dict :", vertex_path_dict[7])
