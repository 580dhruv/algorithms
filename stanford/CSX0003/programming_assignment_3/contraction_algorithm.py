...
def load_data(file_name):
    file = open(file_name,'r')          # opening the file in reading mode
    data = file.readlines()             # reading lines from the file opened
    listed_data = [[int(value) for value in row.strip().split('\t')] for row in data]   # converting the row data into list
    return (listed_data)                       # return the listed data
...
def create_list(data):
    ...
    for vertice in vertices_list:
        for connected_vertice in data[vertice-1]:
...
def contracting_graph(vertices_list,edges_list,edge_vertice_dict,vertices_edge_dict):
    ...
    while len(vertices_list)>2:                                              # continue if there are more than 2 vertices
        picked_edge = edges_list[random.randint(0,len(edges_list)-1)]          # pick a remaining edge (u,v) uniformly at rando
...
def contraction_epochs(data,epochs = 1000):
    # initializing variables
    min_cut_history = {}
    min_cut = len(data)
